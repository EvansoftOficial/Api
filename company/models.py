...
class Company(models.Model):
	...
	certificate_days_left = models.CharField(max_length = 7, null = True, blank = True)

	@staticmethod
	def get_employee_serialized(branch):
	    try:
	        serialized_employee = serializers.serialize('json', [branch])
	        return json.loads(serialized_employee)[0]
	...
	def login_user_comapny(cls, data):
		...
		try:
			...
			all_branch = Branch.list_branch_company(company)
		except Exception as e:
			raise e

		if company is not None:
			_data = cls.get_employee_serialized(company)
			_data['fields']['logo'] = env.URL_LOCAL + _data['fields']['logo']
			_data['all_branch'] = all_branch
			_data['url_seller'] = f"http://104.248.63.144:8080/sellerlogin/{company.documentI}"
...
class Branch(models.Model):
	...
	def create_branch(cls,data):
		...
		try:
			...
			Category.create_category({'LICOR',branch.pk})
			Category.create_category({'GENERAL',branch.pk})
			Category.create_category({'VINO',branch.pk})
			Category.create_category({'CIGARRILLO',branch.pk})
			Category.create_category({'CERVEZA',branch.pk})
			from inventory.models import SubCategory
			SubCategory.create_subcategory_branch({'name':'LICOR','name_category':'LICOR'},branch)
			SubCategory.create_subcategory_branch({'name':'GENERAL','name_category':'GENERAL'},branch)
			SubCategory.create_subcategory_branch({'name':'VINO','name_category':'VINO'},branch)
			SubCategory.create_subcategory_branch({'name':'CIGARRILLO','name_category':'CIGARRILLO'},branch)
			SubCategory.create_subcategory_branch({'name':'CERVEZA','name_category':'CERVEZA'},branch)
	...
	def add_branch(cls,data):
		...
		if branch is None:
			try:
				...
				result = True
				message = "Success"
				result = License.create_license(data, _branch)
				Resolution.create_resolution(data, _branch)
				cls.Create_Resolutions(4, "NC", data, _branch)
				cls.Create_Resolutions(14, "TRAS", data, _branch)
				cls.Create_Resolutions(16, "NP", data, _branch)
				from customer.models import Customer as c
				c.create_consumidor_final(_branch)
				from inventory.models import Supplier as s
				s.create_supplier_general(_branch)
				from inventory.models import Category
				Category.create_category({'name':'LICOR','pk_branch':pk_branch})
				Category.create_category({'name':'GENERAL','pk_branch':pk_branch})
				Category.create_category({'name':'VINO','pk_branch':pk_branch})
				Category.create_category({'name':'CIGARRILLO','pk_branch':pk_branch})
				Category.create_category({'name':'CERVEZA','pk_branch':pk_branch})
				from inventory.models import SubCategory
				SubCategory.create_subcategory_branch({'name':'LICOR','name_category':'LICOR'},_branch)
				SubCategory.create_subcategory_branch({'name':'GENERAL','name_category':'GENERAL'},_branch)
				SubCategory.create_subcategory_branch({'name':'VINO','name_category':'VINO'},_branch)
				SubCategory.create_subcategory_branch({'name':'CIGARRILLO','name_category':'CIGARRILLO'},_branch)
				SubCategory.create_subcategory_branch({'name':'CERVEZA','name_category':'CERVEZA'},_branch)
				SubCategory.create_subcategory_branch({'name':'GASEOSA','name_category':'GASEOSA'},_branch)
			except Exception as e:
				message = str(e)
		return {'result':result, 'message':message}

	@staticmethod
	def Create_Resolutions(type_document_id, prefix, data, branch):
...
class Resolution(models.Model):
	...
	def get_number(cls, type_document,branch):
		resolution = cls.objects.get(type_document_id = type_document, branch = branch)
		return resolution
